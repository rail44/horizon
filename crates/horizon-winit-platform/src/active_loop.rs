...
//! core structural finding of the spike this crate ports: see
//! docs/research/winit-backend-spike.md §5 and docs/winit-backend-design.md.
//!
//! The workaround, unchanged from the spike: stash a raw pointer to the
//! `ActiveEventLoop` in a thread-local for the duration of each callback
//! (`ActiveLoopGuard`), and let `Platform::open_window` borrow it back out
//! (`with_active_loop`). Safety rests entirely on: (1) everything here runs
//! on winit's single event-loop thread, (2) the guard is dropped before the
//! callback that created it returns, so the pointer never survives past the
//! borrow that produced it. Considered and rejected: deferring window
//! creation to the next `ApplicationHandler` callback instead of this
//! thread-local stash — gpui's `open_window` contract is synchronous (it
//! returns `Box<dyn PlatformWindow>` directly to the caller, which may
//! immediately read back its bounds/scale factor), so there is no callback
//! seam to defer into without breaking that contract; the spike's finding
//! (docs/research/winit-backend-spike.md §5) that this is safe for every
//! callback entry point winit offers still holds.
