//! Minimal PTY session wiring for the spike: a stripped-down replica of
//! `horizon::terminal::session::TerminalSession::spawn` (no trace tap, no
//! Horizon config/env plumbing). Bytes in from the PTY reader thread,
//! `TerminalCommand`s demuxed by the writer thread, frames out over
//! `TerminalUpdate` — all through `horizon-terminal-core`'s session loop.
