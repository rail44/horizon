//! `PlatformDispatcher` backed by winit's `EventLoopProxy`.
//!
...
//! on the event-loop thread. This is exactly the mapping the task brief
//! anticipated, and it worked without surprises.
//!
...
//! which is fine for a spike but would need replacing for production use.
    ...
    pub(crate) fn new(proxy: EventLoopProxy<WinitUserEvent>) -> Self {
        ...
        let background_threads = (0..BACKGROUND_THREADS)
            .map(|i| {
                ...
                thread::Builder::new()
                    .name(format!("winit-spike-worker-{i}"))
                    .spawn(move || {
