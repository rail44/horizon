...
(検証時最新 0.18.3)を実測評価する。スパイクコード一式は `remoc-spike` ブランチの
`spike/remoc/` に保存されている(mainには含めない独立クレート)。
...
- フレーム合成(`remoc-spike:spike/remoc/src/frames.rs`): 80x24 と 200x50、行ごとに複数スパン、
...
```sh
git worktree add --detach ../horizon-remoc-spike origin/remoc-spike
cd ../horizon-remoc-spike/spike/remoc
...
`remoc-spike:spike/remoc/tests/skew.rs`。V1/V2 型ペアは同ブランチの `src/skew.rs`(TerminalCommand の運用を模した
...
**裏取り実験**(`remoc-spike:spike/remoc/tests/cross_version.rs`: crates.io の remoc **0.16.1** と
...
`remoc-spike:spike/remoc/src/svc.rs` + 同ブランチの `tests/ergonomics.rs`。`#[rtc::remote] trait TerminalService` の
