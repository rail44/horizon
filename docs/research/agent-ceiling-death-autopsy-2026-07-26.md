...
## 追補 4（2026-07-27）: 修正一式の in vivo 検証（シリーズ最終走）

task 改名 + 委譲 routing 条項 + cap 部分報告 + max_tokens 32,768 +
引数二重エンコード修復 + 「task は調査専用」文言、を全て載せた M3 再走
（session f89f0780）。

| 軸 | M3 素の走行 | 本走行 |
|---|---|---|
| 最初の一手 | bash | **task 委譲** |
| calls/req | 0.99 | **1.57**（序盤 2.4、編集バッチ ×3 も出現） |
| first edit | 63% 地点 | **18% 地点** |
| 実装の再委譲 | —（前走で 2 回） | **0 回** |
| 検証 | 末期 3 回 | **bash 30 回を実装と交互に、最終 check exit 0** |
| 死亡 | 400 @196.3k | **400 @229,207**（= 262,144 − 32,768、予測通りの新天井） |
| 成果 | 9 ファイル +394（壊れ） | **30 ファイル +1070/−154、コンパイル通過状態** |

- template 死は再発せず。二重エンコード起因のセッション死クラスは消滅
- 「調査は委譲 → 報告を地図に batched read → 自分で実装 → 逐次検証」
  という参照形がフルで出現した初の走行
...
1. **polling 病理** — 序盤 ~100 往復が task_output 連打（66 回、
   ...
   polling 検出が修正候補。
2. **cap チェックポイント相転移の「再現（n=2）」は誤りだった** —
   事後のイベント精査（2026-07-28 ハーネス調査）で、この走行の
   polling→実装の切替（r72→r73）は cap でなく**最後の子の完了通知**が
   駆動しており、3 回の cap 前後でツール構成は変化していないと判明。
   相転移の実証は最初の M3 走（continue 直後 r104 で初編集）の n=1 の
   まま。cap/doom 停止は model に何も注入されず、モデルは停止を
   知覚できない（改善候補 8 参照）。
