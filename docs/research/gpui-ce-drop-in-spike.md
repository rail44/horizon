...
## 8. 採用した方針(2026-07-12、オーナー決定)

本スパイクの結果を受けて、**「動く組み合わせを今探す」案は見送り、mainline の
zed pin に留まりつつ移行トリガーだけを定めておく**方針を採用した。理由:
現時点の gpui-ce には固有機能がなく、古い gpui-ce rev への固定は「latest
追従」という彼らのポリシーのサポート外の使い方になる一方、私たちが欲しい
自由(カスタムウィンドウバックエンド等)は mainline が公開する
`Application::with_platform` + `Platform` trait で fork なしに得られるため。

再評価トリガー(いずれかの観測で再検討):

1. **gpui-ce に実利が生える** — Horizon が欲しい機能(libdecor/winit 統合、
   Linux windowing 改善など)が gpui-ce 側にだけ入った時。
2. **gpui-component が追いつく/公式対応する** — upstream API への追従で
   drop-in がコンパイル可能になるか、feature flag での公式対応
   (longbridge discussion #1856)が入った時。
3. **本家が組み込み利用に不利になる** — `with_platform` や modular crate
   構成を壊す方向の変更が入った時(この時 gpui-ce は「保険」から「避難先」
   に変わる)。

関連して、ピンの脆さ(§3 の手法上の注意)は gpui-ce と無関係に
Cargo.toml への明示 `rev =` ピンで塞ぐ(backlog-29)。
