...
- **Model switcher live: the chip is the picker's entry point, and the
  ...
  - **Precedence reversed: session model wins on divergence.** The
    ...
    to a provider entry that `Reload Config` removed mid-session still
    gets its echo but fails to apply at the next turn boundary — the chip
    shows the echoed id while the turn runs on the old one; the daemon
    surfaces the failure as an `Event::Error` in the transcript, and a
    future distinction between switch-echoes and spawn-announcements could
    tighten this if it matters in practice.
