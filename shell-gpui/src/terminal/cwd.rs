...
//! mechanism.
//!
//! Deliberately duplicated from the Floem shell's
//! `src/terminal/session/cwd.rs` rather than shared through a common
//! crate: `docs/gpui-migration-design.md`'s retirement plan deletes the
//! whole Floem shell (this module's twin included) once the GPUI shell
//! replaces it, so a shared crate today would just be unwound later. Keep
//! both copies in sync until then.
