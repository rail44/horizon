...
use crate::agent::event_log::AgentEventLogRecord;

use super::{schema::CLEAR_ALL_AGENT_STATE_SQL, session_id_text, DuckDbAgentStateStore};

impl DuckDbAgentStateStore {
    pub fn replace_from_event_log_records(
        &self,
        records: impl IntoIterator<Item = AgentEventLogRecord>,
    ) -> Result<()> {
    ...
    fn insert_event_log_record(&self, record: AgentEventLogRecord) -> Result<()> {
