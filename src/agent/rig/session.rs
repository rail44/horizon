...
use crate::{
    agent::{
        AgentCommand, AgentEvent, AgentMessage, AgentMessageRole, AgentProviderEvent,
        AgentSessionHandle, AgentSessionState, AgentToolCallResult, StartAgentSession,
    },
...
pub(super) fn spawn_rig_session(
    ...
    thread::spawn(move || {
        ...
        let _ = events_tx.send(AgentEvent::StateChanged(AgentSessionState::WaitingForUser).into());

        while let Ok(command) = commands_rx.recv() {
            match command {
                AgentCommand::Initialize(_) => {
                    let _ =
                        events_tx.send(AgentEvent::StateChanged(AgentSessionState::Running).into());
                    let _ = events_tx
                        .send(AgentEvent::StateChanged(AgentSessionState::WaitingForUser).into());
                }
...
fn handle_user_message(
    ...
    events_tx: &crossbeam_channel::Sender<AgentProviderEvent>,
    text: String,
) {
    let _ = events_tx.send(AgentEvent::StateChanged(AgentSessionState::Running).into());
    let _ = events_tx.send(
        AgentEvent::MessageCommitted(AgentMessage {
