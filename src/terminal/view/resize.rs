...
/// `ResizeDebounce` again, so a single lost timer firing loses the final
/// size permanently rather than merely delaying it. Forwarding immediately
/// during this initial window sidesteps the timer for exactly the frames
/// where that matters; a live window/pane drag (the scenario
